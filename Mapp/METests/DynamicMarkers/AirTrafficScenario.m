...
- (id) init
{
	if(self=[super init])
	{
		...
		self.uid = 0;
		self.maxObjects = 200;
		self.movingObjects = [[[NSMutableArray alloc]init]autorelease];
...
{
	int lon = -[self randomDouble:-RDU_COORD.longitude max:-SFO_COORD.longitude];
	int lat = [self randomDouble:15 max:75];
...
- (MovingObject*) newObject
{
	...
	MovingObject* movingObject = [[[MovingObject alloc]init]autorelease];
	movingObject.heading = [self randomDouble:0 max:360];
	...
	movingObject.velocity = [self randomDouble:100000 max:500000];
...
- (void) trackObject:(MovingObject*) objectToTrack
{
	...
	double beaconAnimationDuration = 1.0;
	...
	for(MovingObject* movingObject in self.movingObjects)
	{
		if(movingObject.alive)
		{
			...
			[self.meMapViewController updateDynamicMarkerRotation:self.name
													   markerName:movingObject.name
														 rotation:movingObject.heading animationDuration:0.2];
